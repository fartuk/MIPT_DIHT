...
class BigInt{
	public:
		...
		BigInt operator/(BigInt& b){
				...
				int& ptNumRf = ptNum;
				int& ptInsRf = ptIns;
				normalize(redDivRf, thisRf, redB.getSize(), ptNumRf, ptInsRf);
				tmpDiv = redDiv - redB;
				std::cout << "\n _________redDiv\n";
				redDiv.print();
				std::cout << "\n _________redB\n";
				...
				tmpB.print();
				std::cout << "\n";
				for(int i = 0; i < tmpB.getSize(); i++){
						std::cout << tmpB.vect[i] << "\n";
				}
				...
				redDiv.print();
				std::cout << "\n \n";
				for(int i = 0; i < redDiv.getSize(); i++){
						std::cout << redDiv.vect[i] << "\n";
				}

				redDiv = redDiv - tmpB; 
				std::cout << "\n _________redDiv - tmpB\n";
				redDiv.print();
...
*/
		...
		friend void normalize(BigInt& norm, BigInt& donor, int quan, int& num, int& ins);
...
void normalize(BigInt& norm, BigInt& donor, int quan, int& num, int& ins){
		BigInt tmp;
		tmp = norm;
		tmp.vect[0] = 1;
		int k = 0;
